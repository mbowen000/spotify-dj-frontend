...
if(process.env.MONGOLAB_URI) {
	var mongojsconn = mongojs(process.env.MONGOLAB_URI, ['tracks', 'tracks-2', 'rooms']);	
	
	var db; // actually "tracks" -- shoudl refactor
	var dbRooms;

	if(process.env.collection) {
		console.log('got connection override from envs: ' + process.env.collection);
		db = mongojsconn[process.env.collection];
	}
	else {
...
app.get('/room/:id?', function(req, res) {
	...
	// }
	// else {
		dbRooms.find({}, function(err, results) {
			if(err) {
...
app.get('/queue', function (req, res) {
  ...
  db.find({}).count(function(err, count) {
  	...
  	db.find({room: req.query.room.Id}).sort({upvoteCount: -1, lastPlayedDate: 1, dateAdded: 1}).limit(numPerPage).skip(offset, function(err, docs) {
	  	if(err) {
	  		res.status(500).send(err);
	  	}
