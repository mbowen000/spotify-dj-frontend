...
if(process.env.MONGOLAB_URI) {
	var mongojsconn = mongojs(process.env.MONGOLAB_URI, ['tracks']);	
...
app.get('/queue', function (req, res) {
  res.type('json');
  db.find({}).sort({upvoteCount: -1, lastPlayedDate: 1}, function(err, docs) {
  	if(err) {
  		res.status(500).send(err);
  	}
...
app.post('/track/upvote', function(req, res) {
	...
	db.find({ id: req.body.track.id }, function(err, tracks) {
		...
		if(duplicateFound) {
			res.status(401).send('Cannot upvote multiple times');
			return false;
		}
		else {
